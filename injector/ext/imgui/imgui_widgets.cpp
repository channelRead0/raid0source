...
#include <src/globals/structures/color.hpp>
#include <map>
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    static std::map< ImGuiID, ButtonAnimations > Animations;
    auto AnimationMap = Animations.find(AnimationID);
    if (AnimationMap == Animations.end())
    {
        Animations.insert({ AnimationID, ButtonAnimations() });
        AnimationMap = Animations.find(AnimationID);
    }

    AnimationMap->second.BorderColor.Value = ImLerp(AnimationMap->second.BorderColor.Value, hovered ? Color("7A79A8").GetColorVec4() : Schemes::ColorSchemeUI.Border.GetColorVec4(), ImGui::GetIO().DeltaTime * 7.F);
    AnimationMap->second.TextColor.Value = ImLerp(AnimationMap->second.TextColor.Value, hovered ? Color("7A79A8").GetColorVec4() : Schemes::ColorSchemeUI.TextDisabled.GetColorVec4(), ImGui::GetIO().DeltaTime * 7.F);

    window->DrawList->AddShadowRect(bb.Min, bb.Max, AnimationMap->second.BorderColor, 10.F, ImVec2(0.F, 0.F), 0, 0.F);
    window->DrawList->AddRectFilled(bb.Min, bb.Max, Schemes::ColorSchemeUI.WindowBackground, 0.F);
    window->DrawList->AddRect(bb.Min, bb.Max, AnimationMap->second.BorderColor, 0.F);
    // Render
  /*  const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding);*/
    window->DrawList->AddText(ImVec2(bb.GetCenter() - (ImGui::CalcTextSize(label) / 2.F)) - ImVec2(0.F, 1.F), AnimationMap->second.TextColor, label);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = ImVec2(245.f, 19.F);/*CalcItemSize(size_arg, CalcItemWidth(), (is_multiline ? g.FontSize * 8.0f : label_size.y) + style.FramePadding.y * 2.0f);*/ // Arbitrary default of 8 lines high for multi-line
    const ImVec2 total_size = ImVec2(frame_size.x, frame_size.y);


    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0.F, ImGui::CalcTextSize(label).y + 2.F), window->DC.CursorPos + frame_size + ImVec2(0.F, ImGui::CalcTextSize(label).y + 2.F));
    const ImRect total_bb(frame_bb.Min -  ImVec2(0.F, ImGui::CalcTextSize(label).y + 2.F), frame_bb.Min + total_size);
    ...
    {
        //RenderNavHighlight(frame_bb, id);
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, Schemes::ColorSchemeUI.WindowBackground, 1.F);
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, Schemes::ColorSchemeUI.Border, 1.F);
    }
    ...
    struct TextBoxAnimation
    {
        float X = 0.F;
        float SelectionX = 0.F;
        float SelectionAlpha = 0.F;
        ImColor CursorColor;
    };

    const int AnimationID = id;

    static std::map< ImGuiID, TextBoxAnimation > Animations;
    auto AnimationMap = Animations.find(AnimationID);
    if (AnimationMap == Animations.end())
    {
        Animations.insert({ AnimationID, TextBoxAnimation() });
        AnimationMap = Animations.find(AnimationID);
    }
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    AnimationMap->second.SelectionX = ImLerp(AnimationMap->second.SelectionX, rect_size.x, ImGui::GetIO().DeltaTime * 7.F);
                    AnimationMap->second.SelectionAlpha = ImLerp(AnimationMap->second.SelectionAlpha, 150.F, ImGui::GetIO().DeltaTime * 7.F);

                    if (rect_size.x <= 0.0f) rect_size.x = IM_TRUNC(g.Font->GetCharAdvance((ImWchar)' ') * 0.50f); // So we can see selected empty lines

                    ImRect rect(rect_pos + ImVec2(0.0f, bg_offy_up - g.FontSize), rect_pos + ImVec2(AnimationMap->second.SelectionX, bg_offy_dn));
                    rect.ClipWith(clip_rect);
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min, rect.Max - ImVec2(0.F, 2.F), Color("444751").SetAlpha(AnimationMap->second.SelectionAlpha));
                }
        ...
        if (!render_selection)
        {
            AnimationMap->second.SelectionX = 0.F;
            AnimationMap->second.SelectionAlpha = 0.F;
        }
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll, Color("444751"), buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
        ...
        if (render_cursor)
        {
            ...
            AnimationMap->second.X = ImLerp(AnimationMap->second.X, cursor_screen_pos.x, ImGui::GetIO().DeltaTime * 7.F);
            const auto RenderPos = ImRect(AnimationMap->second.X, frame_bb.Min.y + 1.5F, AnimationMap->second.X + 1.0f, frame_bb.Max.y - 2.F);

            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(RenderPos.Min, RenderPos.GetBL(), Color("444751"));
    ...
    else
    {
        ...
        {
            ImU32 col = GetColorU32(is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text);
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos, Color("444751"), buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    }
    ...
    if (label_size.x > 0) {
        window->DrawList->AddText(ImVec2(frame_bb.Min.x, frame_bb.Min.y - 2.F - ImGui::CalcTextSize(label).y), Color("444751"), label);
