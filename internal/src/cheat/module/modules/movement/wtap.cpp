...
void WTap::OnTick(JNIEnv* Env)
{
	...
	auto ReleaseItem = [Env]() -> void
	{
		...
		const auto& ForwardBind = GameSettings->GetKeyBindForwardPtr(Env);

		ForwardBind->SetPressed(Env, true);
		WasSet = false;
	};
	...
	auto UseItem = [Env](DWORD ItemReleaseTime) -> void
	{
		...
		ForwardBind->SetPressed(Env, false);
...
void WTap::OnHandleStatusUpdate(JNIEnv* Env, const std::shared_ptr<Player>& TargetInstance, jbyte Byte, bool* ShouldCancel)
{
	if (!this->GetState()) {
		return;
	}
